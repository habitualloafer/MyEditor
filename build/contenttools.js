...
(function() {
    ...
    ContentTools = {
        ...
        DEFAULT_TOOLS: [['bold', 'italic', 'link', 'align-left', 'align-center', 'align-right'], ['heading', 'subheading', 'paragraph', 'unordered-list', 'ordered-list', 'table', 'indent', 'unindent', 'line-break'], ['image', 'video', 'preformatted'], ['undo', 'redo', 'remove','color','bgcolor']],
    ...
    //hodo SectionUI
    ContentTools.SectionUI=(function(_super) {
        __extends(SectionUI, _super);

        function SectionUI(sectionName, listArr) {
            SectionUI.__super__.constructor.call(this);
            this.sectionName=sectionName;
            this.listArr=listArr;
        }

        SectionUI.prototype.mount = function() {
            SectionUI.__super__.mount.call(this);
            this._domElement = this.constructor.createDiv(['hd-editor']);
            console.log(this.parent())
            this.parent().domElement().appendChild(this._domElement);
            var inithtml='<div class="hd-editor-nav">'+this.sectionName+'</div>';
            function mount(listArr) {
                for(var i=0;i<listArr.length;i++){
                    inithtml+='<div class="hd-editor-list"><span class="list-hover"></span><span class="list-img"></span><span class="list-text">'+listArr[i]+'</span></div>';
                }
                var _domElement=document.getElementsByClassName('hd-editor')[0];
                _domElement.innerHTML=inithtml;
            }
            mount(this.listArr);
            function stylehover() {
                var editList = document.getElementsByClassName('hd-editor-list');
                var listHover = document.getElementsByClassName('list-hover');
                for (var i = 0; i < editList.length; i++) {
                    editList[i].addEventListener('mousemove', (function (i) {
                        return function () {
                            listHover[i].style.backgroundColor = 'red';
                        }
                    })(i))
                    editList[i].addEventListener('mouseout', (function (i) {
                        return function () {
                            listHover[i].style.backgroundColor = 'orange';
                        }
                    })(i))
                }
            }
...
//hodo own
    ...
    _EditorApp = (function(_super) {
        ...
        _EditorApp.prototype.init = function(queryOrDOMElements) {
            var allDataName = document.querySelectorAll(queryOrDOMElements);//根据参数检索全文中带有参数的属性的元素
            for (var i = 0; i < allDataName.length; i++) {//遍历所有元素，监听每个元素的click事件
                allDataName[i].addEventListener('click', (function (i) {
                    return function () {
                        var dataName=allDataName[i].getAttribute('data-name');//获取点击元素的data-name值
...
//hodo
    ContentTools.Tools.Color=(function (_super) {
        ...
        Color.apply = function(element, selection, callback) {
            var alignmentClassNames, className, _i, _len, _ref,elements;
            if ((_ref = element.type()) === 'ListItemText' || _ref === 'TableCellText') {
                element = element.parent();
            }
            alignmentClassNames = [ContentTools.Tools.AlignLeft.className, ContentTools.Tools.AlignCenter.className, ContentTools.Tools.AlignRight.className];
            ...
            ContentTools.hex.show();
            ColorPicker(
                document.getElementsByClassName('cp-default')[0],
                function(hex, hsv, rgb) {
                    element.attr('style',"background-color:"+ContentTools.bgcolor+';'+"color:"+hex);
                    ContentTools.color=hex;
                }
            );
            return callback(true);
        };
    ...
    //背景颜色
    ContentTools.Tools.BackgroundColor=(function (_super) {
        __extends(BackgroundColor, _super);

        function BackgroundColor() {
            return BackgroundColor.__super__.constructor.apply(this, arguments);
        }
        ContentTools.ToolShelf.stow(BackgroundColor, 'bgcolor');
        BackgroundColor.label='bgcolor';
        BackgroundColor.icon='bgcolor';
        BackgroundColor.className='bgcolor';
        BackgroundColor.canApply = function(element, selection) {
            return element.content !== void 0;
        };

        BackgroundColor.apply = function(element, selection, callback) {
            var alignmentClassNames, className, _i, _len, _ref,elements;
            if ((_ref = element.type()) === 'ListItemText' || _ref === 'TableCellText') {
                element = element.parent();
            }
            ...
            ColorPicker(
                document.getElementsByClassName('cp-default')[0],
                function(hex, hsv, rgb) {
                    element.attr('style',"background-color:"+hex+';'+"color:"+ContentTools.color);
                    ContentTools.bgcolor=hex;
                }
            );
            return callback(true);
        };
    ...
    /*  ContentTools.Tools.Red=(function (_super) {
     ...
     Red.className='red';
     return Red;
     })(ContentTools.Tools.Green);
     ContentTools.Tools.Yellow=(function (_super) {
     __extends(Yellow, _super);
     function Yellow() {
     return Yellow.__super__.constructor.apply(this, arguments);
     }
     ContentTools.ToolShelf.stow(Yellow, 'yellow');
     Yellow.label='yellow';
     Yellow.icon='yellow';
     Yellow.className='yellow';
     return Yellow;
     })(ContentTools.Tools.Green);
     ContentTools.Tools.Pink=(function (_super) {
     __extends(Pink, _super);
     function Pink() {
     return Pink.__super__.constructor.apply(this, arguments);
     }
     ContentTools.ToolShelf.stow(Pink, 'pink');
     Pink.label='pink';
     Pink.icon='pink';
     Pink.className='pink';
     return Pink;
     })(ContentTools.Tools.Green);
     ContentTools.Tools.Gray=(function (_super) {
     __extends(Gray, _super);
     function Gray() {
     return Gray.__super__.constructor.apply(this, arguments);
     }
     ContentTools.ToolShelf.stow(Gray, 'gray');
     Gray.label='gray';
     Gray.icon='gray';
     Gray.className='gray';
     return Gray;
