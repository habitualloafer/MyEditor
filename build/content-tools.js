document.write('<link href="./build/jquery-ui.css" rel="stylesheet">');
document.write('<script src="./build/jquery.js"></script>');
document.write('<script src="./build/jquery-ui.js"></script>');
document.write('<script src="./build/draggabilly.pkgd.min.js"></script>');
//color picker插件
...
(function() {
  ...
  ContentEdit.Element = (function(_super) {
    ...
    Element.prototype.mount = function() {
      ...
      if (sibling) {
        this.parent().subElement().insertBefore(this._domElement, sibling.domElement());
      } else {
        if (this.isFixed()) {
          this.parent().subElement().parentNode.replaceChild(this._domElement, this.parent().domElement());
          this.parent()._domElement = this._domElement;
        } else {
          this.parent().subElement().appendChild(this._domElement);
        }
      }
  ...
  ContentEdit.Region = (function(_super) {
    ...
    Region.prototype.domElement = function() {
      ...
      Region.prototype.subElement = function() {
    ...
    /*Region.prototype.setContent = function(domElementOrHTML) {
      ...
      for (_j = 0, _len1 = childNodes.length; _j < _len1; _j++) {
        ...
        element = cls.fromDOMElement(childNode);//相当于克隆元素
    ...
    Region.prototype.setContent = function(domElementOrHTML) {
      var c, child, childNode, childNodes, cls, domElement, element, tagNames, wrapper, _i, _j, _len, _len1, _ref,that=this,childSet=[],elementSet={},addressSet={};
      domElement = domElementOrHTML;
      /*if (domElementOrHTML.childNodes === void 0) {
        wrapper = document.createElement('div');
        wrapper.innerHTML = domElementOrHTML;
        ...
        this._addressSet=domElementOrHTML;
      (function (element) {
          var arr=[];
          done(element);
          function done(node){
              if(node.children.length!=0) {
                  ...
                  for (var index = 0; index < childrenNodes.length; index++) {
                      if(childrenNodes[index].hasAttribute('data-type')){
                          if(childrenNodes[index].getAttribute('data-type')!='img'){
        ...
        (function (element) {
            ...
            function done(node){
                if(node.children.length!=0) {
                    var childrenNodes = node.children;
                    for (var index = 0; index < childrenNodes.length; index++) {
                        if(childrenNodes[index].hasAttribute('data-region-num')){
                                arr.push(childrenNodes[index]);
                        }
                        done(childrenNodes[index]);
                    }
        ...
        for(var i=0;i<this._regionSet.length;i++){
            var num=this._regionSet[i].getAttribute('data-region-num');
            (function (element) {
                done(element);
                function done(node){
                    if(node.children.length!=0) {
                        ...
                        for (var index = 0; index < childrenNodes.length; index++) {
                            ...
                            if(childrenNodes[index].hasAttribute('data-type')){
                                if(childrenNodes[index].getAttribute('data-type')!='img'){
                                    name=childrenNodes[index].getAttribute('data-name');
                                    arr.push(num);
                                    arr.push(name);
                                   childSet.push(arr)
                                }
      ...
      for (_j = 0, _len1 = childNodes.length; _j < _len1; _j++) {
        ...
        var parentElement=childNode.parentNode;
        element = cls.fromDOMElement(childNode);//相当于克隆元素
          if(elementSet[this._childArr[_j][0]]==undefined){
              elementSet[this._childArr[_j][0]]={};
              elementSet[this._childArr[_j][0]][this._childArr[_j][1]]={};
              elementSet[this._childArr[_j][0]][this._childArr[_j][1]]=element;
              /*addressSet[this._childArr[_j][0]]={};
              addressSet[this._childArr[_j][0]][this._childArr[_j][1]]={};
              addressSet[this._childArr[_j][0]][this._childArr[_j][1]]=parentElement;*/
          }
          else {
              elementSet[this._childArr[_j][0]][this._childArr[_j][1]]={};
              elementSet[this._childArr[_j][0]][this._childArr[_j][1]]=element;
          ...
          this._subElement=parentElement;
  ...
  ContentEdit.Text = (function(_super) {
    ...
    Text.prototype.focus = function(supressDOMFocus) {
      if (this.isMounted()) {
       ...
       /* this._domElement.setAttribute('contenteditable', '');*/
          setTimeout(function () {
              that._domElement.setAttribute('contenteditable', '');
          },100)
      }
    ...
    Text.prototype.mount = function() {
      ...
      this._domElement = document.createElement(this._tagName);
        ...
        this._domElement.addEventListener('click',(function(){
            return function(ev){
                ...
                clickevent=setTimeout(function () {
                    var control=ContentTools.EditorApp.get();
                    control.setSingleElement(that._domElement);
                    control.setSingleElementType(that._domElement.getAttribute('data-type'));
                    control.controlstate('element');
                    control.controller();
        ...
        this._domElement.addEventListener('dblclick',(function(){
            return function(ev){
                ev.preventDefault();
                ev.stopPropagation();
                clearTimeout(clickevent);
                var control=ContentTools.EditorApp.get();
                control.controlstate('texttool');
                control.controller();
...
(function() {
  ...
  ContentTools.ComponentUI = (function() {
    ...
    ComponentUI.createDiv = function(classNames, attributes, content) {
      ...
      /*hodo*/
    ComponentUI.prototype.sectionNames=function(){
        var section;
        section=document.querySelectorAll('*[data-section]');
    };
    ComponentUI.createSpan = function(classNames, attributes, content) {
          var spanElement, name, value;
          spanElement = document.createElement('span');
          if (classNames && classNames.length > 0) {
              spanElement.setAttribute('class', classNames.join(' '));
          }
          if (attributes) {
              for (name in attributes) {
                  value = attributes[name];
      ...
      ComponentUI.createInput = function(classNames, attributes, content,bool) {
          var domElement, name, value,thebool;
          var that=this;
    ...
    ComponentUI.createDl=function(dlClassNames,/*dtClassNames,ddClassNames,*/ attributes, content) {
        var dlElement,name, value,dtElement,ddElement;
        dlElement = document.createElement('dl');
        dtElement=document.createElement('dt');
        ddElement=document.createElement('dd');
        if (dlClassNames && dlClassNames.length > 0) {
            dlElement.setAttribute('class', dlClassNames.join(' '));
        }

        if (attributes) {
            for (name in attributes) {
                value = attributes[name];
                dlElement.setAttribute(name, value);
            }
        }
        if (content) {
    ...
    ComponentUI.createSelect = function(classNames, attributes, arr) {
          var domElement, name, value;
          ...
          if (arr) {
              for(var i=0;i<arr.length;i++){
                  domElement.appendChild(this.createOption(null,null,arr[i]));
              }
          }
          return domElement;
      };

      ComponentUI.createOption = function(classNames, attributes, content) {
          var domElement, name, value;
          domElement = document.createElement('option');
          if (classNames && classNames.length > 0) {
              domElement.setAttribute('class', classNames.join(' '));
          }
          if (attributes) {
          ...
          if (content) {
              domElement.innerHTML = content;
              domElement.value='_'+content.toLowerCase();
          }
          return domElement;
      };

    ComponentUI.createNavStyle=function (navName) {
        var domElement,spanElement;
        domElement=this.createDiv(['hd-editor-nav'],null,navName);
        spanElement=this.createSpan(['hd-editor-nav-exit'],null,'&times;');
        spanElement.addEventListener('click',(function () {
            return function (ev) {
                ev.stopPropagation();
                var control=ContentTools.EditorApp.get();
                control.controlstate('exitstate');
                control.controller();
            }
        })());
    ...
    ComponentUI.createListStyle=function (listName) {
        var domElement,spanElement1,spanElement2,spanElement3;
        domElement=this.createDiv(['hd-editor-list']);
        spanElement1=this.createSpan(['list-hover']);
        spanElement2=this.createDiv(['list-img']);
        spanElement3=this.createSpan(['list-text'],null,listName);
        domElement.appendChild(spanElement1);
        domElement.appendChild(spanElement2);
        domElement.appendChild(spanElement3);
        ...
        domElement.addEventListener('mouseout',function () {
            spanElement1.style.backgroundColor='orange';
        });
        return domElement;
      };

    ComponentUI.createListNavStyle=function (listNavName) {
        var domElement,spanElement1,spanElement2;
        domElement=this.createDiv(['hd-editor-listnav'],null,listNavName);
        spanElement1=this.createSpan(['hd-editor-nav-back'],null,'&lt;');
        spanElement1.addEventListener('click',(function (_this) {
            return function (ev) {
                ev.preventDefault();
                var control=ContentTools.EditorApp.get();
                control.controlstate('returnstate');
                control.controller();
            }
        })(this));
        domElement.appendChild(spanElement1);
        spanElement2=this.createSpan(['hd-editor-nav-exit'],null,'&times;');
        spanElement2.addEventListener('click',(function (_this) {
            return function (ev) {
                ev.preventDefault();
                var control=ContentTools.EditorApp.get();
                control.controlstate('exitstate');
    ...
    ComponentUI.prototype.createRegionbox=function (regionNum) {
        var domElement,spanElement1,spanElement2,divElement1,divElement2,dlElement1,dlElement2,dlElement3,dlElement4;
        domElement=this.constructor.createDiv(['hd-editor-handle_region']);
        divElement1=this.constructor.createDiv(['region_name'],null,'Region '+regionNum);
        divElement2=this.constructor.createDiv(['regions_ctrl']);
        spanElement1=this.constructor.createSpan(['region_left'],null,'&lt;');
        spanElement2=this.constructor.createSpan(['region_right'],null,'&gt;');
        dlElement1=this.constructor.createDl(null,null,'Move Back');
        dlElement2=this.constructor.createDl(null,null,'Move Next');
        dlElement3=this.constructor.createDl(null,null,'Add');
        dlElement4=this.constructor.createDl(null,null,'Delete');
        divElement1.appendChild(spanElement1);
        divElement1.appendChild(spanElement2);
        divElement2.appendChild(dlElement1);
        divElement2.appendChild(dlElement2);
        divElement2.appendChild(dlElement3);
        divElement2.appendChild(dlElement4);
        domElement.appendChild(divElement1);
        domElement.appendChild(divElement2);
        return domElement;
    };
      /*ComponentUI.createRegionbox=function (regionNum) {

          var domElement,spanElement1,spanElement2,divElement1,divElement2,dlElement1,dlElement2,dlElement3,dlElement4;
          domElement=this.createDiv(['hd-editor-handle_region']);
          divElement1=this.createDiv(['region_name'],null,'Region '+regionNum);
          divElement2=this.createDiv(['regions_ctrl']);
          spanElement1=this.createSpan(['region_left'],null,'&lt;');
          spanElement2=this.createSpan(['region_right'],null,'&gt;');
          dlElement1=this.createDl(null,null,'Move Back');
          dlElement2=this.createDl(null,null,'Move Next');
          dlElement3=this.createDl(null,null,'Add');
          dlElement4=this.createDl(null,null,'Delete');
          divElement1.appendChild(spanElement1);
          divElement1.appendChild(spanElement2);
          divElement2.appendChild(dlElement1);
          divElement2.appendChild(dlElement2);
    ...
    ComponentUI.createTextbox=function (name,textElement,bool,elementSet) {
        var domElement,inputElement,textbox,text,that=this,num;
        textbox=textElement.querySelector('[data-name="'+name+'"]');
        num=textElement.getAttribute('data-region-num');
        if(!bool){
            text=textbox.innerText;
        }
        domElement=this.createDiv(['textbox'],null,name);
        inputElement=this.createInput(null,{'type':'text'},text,bool);
        domElement.appendChild(inputElement);
        if(!bool) {
            domElement.addEventListener('input', function () {
                textbox.innerHTML = inputElement.value;
                elementSet[num][name]._onKeyUp();
            });
            textbox.addEventListener('input', function () {
                inputElement.value = textbox.innerHTML;
            });
    ...
    ComponentUI.createImgbox=function (name,imgElement) {
        var domElement,divElement1,divElement2,divElement3,subDivElement,inputElement,srcvalue,imgbox;
        imgbox=imgElement.querySelector('[data-name="'+name+'"]');
        domElement=this.createDiv(['imgbox']);
        divElement1=this.createTextbox(name,imgElement,true);
        divElement2=this.createDiv(['text-btn'],null,'Add or Upload File');
        divElement3=this.createDiv(['img-region']);
        subDivElement=this.createDiv(['img-delete'],null,'-');
        inputElement=this.createInput(['file-input'],{'type':'file','style':'display:none'},null,false)
        divElement3.appendChild(subDivElement);
        domElement.appendChild(divElement1);
        domElement.appendChild(divElement2);
        domElement.appendChild(divElement3);
         domElement.appendChild(inputElement);
        divElement3.style.backgroundImage=imgbox.style.backgroundImage;
        subDivElement.addEventListener('click',function () {
            divElement3.style.backgroundImage=null;
            imgbox.style.backgroundImage=null;
        });
        return  domElement;
    };
    
    ComponentUI.createBtnbox=function (name,btnElement,elementSet) {
        var domElement,inputElement1,inputElement2,divElement,selectElement,inputElement3,btnbox,num;
        btnbox=btnElement.querySelector('[data-name="'+name+'"]');
        num=btnElement.getAttribute('data-region-num');
        btnbox.target='_blank';
        domElement=this.createDiv(['btnbox']);
        inputElement1=this.createTextbox(name,btnElement,false,elementSet);
        inputElement2=this.createTextbox(name+' Link',btnElement,true,elementSet);
        divElement=this.createDiv(['btn-select'],null,'Target');
        selectElement=this.createSelect(['btn-sel'],null,['Blank','Self','Parent','Top']);
        
        divElement.appendChild(selectElement);
        domElement.appendChild(inputElement1);
        domElement.appendChild(inputElement2);
        domElement.appendChild(divElement);
        inputElement2.addEventListener('input',function () {
            btnbox.href=this.getElementsByTagName('input')[0].value;
        });
        selectElement.addEventListener('change',function () {
            btnbox.target=this.value;
            elementSet[num][name].taint();
        })
       
        return domElement;
    };

    ComponentUI.createColorbox=function (name,colorElement,elementSet) {
        var domElement,divElement1,divElement2,divElement3,divElement4,bool,colorEle,num;
        colorEle=colorElement.querySelector('[data-name="'+name+'"]');
        num=colorElement.getAttribute('data-region-num');
        domElement=this.createDiv(["colorbox"],null,'Section Title Font Color');
        divElement1=this.createDiv(["color-show"]);
        divElement2=this.createDiv(["color-btn"],null,'Select Color');
        divElement1.appendChild(divElement2);
        domElement.appendChild(divElement1);
        bool=true;
        var that=this;
        divElement2.addEventListener('click',function () {
            if(document.getElementsByClassName('colorregion')>0){
                document.removeChild(document.getElementsByClassName('colorregion'));
                divElement2.innerHTML='Select Color';
            }
            if(bool){
                divElement3=that.createDiv(['colorregion']);
                domElement.appendChild(divElement3);
                divElement4=that.createDiv(['cp-default']);
                divElement3.appendChild(divElement4);
                domElement.appendChild(divElement3);
                divElement2.innerHTML='Current Color';
                ColorPicker(
                    domElement.getElementsByClassName('cp-default')[0],
                    function(hex, hsv, rgb) {
                        divElement1.setAttribute('style',"background-color:"+hex);
                        colorEle.style.color=hex;
                        elementSet[num][name].taint();
                    }
                );
                bool=false;
                ...
                domElement.removeChild(divElement3);
                divElement2.innerHTML='Select Color';
                bool=true;
            }
        });
      ...
      ComponentUI.createColorbox2=function (aimElement,aimAttr,colorName,elementSet,regionNum) {
          var domElement,divElement1,divElement2,divElement3,divElement4,bool,colorEle,name;
          name=aimElement.getAttribute('data-name');
          domElement=this.createDiv(["colorbox"],null,colorName);
          divElement1=this.createDiv(["color-show"]);
          divElement2=this.createDiv(["color-btn"],null,'Select Color');
          divElement1.appendChild(divElement2);
          domElement.appendChild(divElement1);
          ...
          divElement2.addEventListener('click',function () {
              ...
              if(bool){
                  ...
                  ColorPicker(
                      domElement.getElementsByClassName('cp-default')[0],
                      function(hex, hsv, rgb) {
                          var time=new ContentEdit.Node();
                          time.taint();
                          divElement1.setAttribute('style',"background-color:"+hex);
                          aimElement.style[aimAttr]=hex;
                          elementSet[regionNum][name].taint();
                      }
                  );
                  bool=false;
    ...
    ComponentUI.SectionName=function () {
        var section,sectionName,dataName,data,dataType;
        var sectionList={};
        section=document.querySelectorAll('*[data-section]');
        var that=this;
        for(var i=0;i<section.length;i++){
            var dataList={};
            sectionName=section[i].getAttribute('data-section');
            data=section[i].querySelectorAll('*[data-name]')
            for(var j=0;j<data.length;j++){
                dataName=data[j].getAttribute('data-name');
                dataType=data[j].getAttribute('data-type');
                if(dataType=='text'){
                    dataList[dataName]=data[j].innerText;
                }
                if(dataType=='img'){
                    dataList[dataName]=data[j].style.backgroundImage;
                }
                if(dataType=='button'){
                    var buttonList={};
                    buttonList.text=data[j].innerText;
                    buttonList.link=data[j].href;
                    buttonList.targ=data[j].target;
                    dataList[dataName]=buttonList;

                }


            }
            sectionList[sectionName]=dataList;
        }
        return sectionList;
    };

    ComponentUI.Slider=function (name,element,elementSet) {
        var domElement,spanElement,divElement,sizeElement,num;
        sizeElement=element.querySelector('[data-name="'+name+'"]');
        num=element.getAttribute('data-region-num');
        domElement=this.createDiv(['slider'],null,'Section Title Font Size');
        spanElement=this.createSpan(null,null,'16');
        divElement=this.createDiv(['range']);
        domElement.appendChild(spanElement);
        domElement.appendChild(divElement);
        $(function() {
            $(divElement).slider({
        ...
        function change() {
            var value=$(divElement).slider("value");
            spanElement.innerText=value;
            sizeElement.style.fontSize=value+"px";
            elementSet[num][name].taint();
        }
        return domElement;
    };

      ComponentUI.Slider2=function (aimElement, aimAttr,sliderName,min,max,init,elementSet,regionNum) {
          var domElement,spanElement,divElement,sizeElement,name;
          name=aimElement.getAttribute('data-name');
          domElement=this.createDiv(['slider'],null,sliderName);
          spanElement=this.createSpan(null,null,init);
          divElement=this.createDiv(['range']);
          domElement.appendChild(spanElement);
          domElement.appendChild(divElement);
          $(function() {
          ...
          function change() {
              var value=$(divElement).slider("value");
              if(aimAttr!=='opacity'&&aimAttr!=='border') {
                  spanElement.innerText=value;
                  aimElement.style[aimAttr] = value + 'px';

                  elementSet[regionNum][name].taint();
              }
              else if(aimAttr=='border'){
                  spanElement.innerText=value;
                 /* aimElement.setAttribute('style','border:1px solid black');*/
                  aimElement.style[aimAttr+'Width'] = value + 'px';
                  aimElement.style.borderStyle='solid';
                  elementSet[regionNum][name].taint();
              }
              else {
                  spanElement.innerText=value/max;
                  aimElement.style[aimAttr] = value/max;
                  elementSet[regionNum][name].taint();
              }
          }
          return domElement;
      };
    ComponentUI.Setting=function (changeElement) {
        var domElement,divElement1,divElement2,divElement3,inputElement1,inputElement2,inputElement3,index,initElement,parentElement;
        index=changeElement.getAttribute('data-section-number');
        domElement=this.createDiv();
        divElement1=this.createDiv(['textbox'],null,'Section Top Padding');
        inputElement1=this.createInput(null,{'type':'text'},'100px',false);
        divElement1.appendChild(inputElement1);
        inputElement1.addEventListener('input',function () {
            changeElement.style.paddingTop=inputElement1.value;
        });
        divElement2=this.createDiv(['textbox'],null,'Section Bottom Padding');
        inputElement2=this.createInput(null,{'type':'text'},'100px',false);
        divElement2.appendChild(inputElement2);
        inputElement2.addEventListener('input',function () {
            changeElement.style.paddingBottom=inputElement2.value;
        });
        divElement3=this.createDiv(['textbox'],null,'Section ID');
        inputElement3=this.createInput(null,{'type':'text'},index,false);
        divElement3.appendChild(inputElement3);
        inputElement3.addEventListener('input',function () {
            var cloneInit=changeElement.cloneNode(true);
            var reg=/^\d+$/;
            if(reg.test(inputElement3.value)==true){
                initElement=document.querySelector('[data-section-number="'+index+'"]');
                parentElement=initElement.parentNode;
                var aimElement=document.querySelector('[data-section-number="'+inputElement3.value+'"]');
                 var cloneAim=aimElement.cloneNode(true);
                 cloneAim.setAttribute('data-section-number',index);
                 cloneInit.setAttribute('data-section-number',inputElement3.value);
                 parentElement.replaceChild(cloneAim,initElement);
                 parentElement.replaceChild(cloneInit,aimElement);
                index=inputElement3.value;
            }
            else {
                return;
            }
  ...
  ContentTools.Event = (function() {
    ...
    ContentTools.HeaderTools=(function (_super) {
        __extends(HeaderTools,_super);

        function HeaderTools(){

        };

        HeaderTools.prototype.mount=function () {
            var domElment,divElement1,divElement2,divElement3,divElement4,divElement5,divElement6,divElement7,divElement8,divElement9,divElement10,spanElement1,spanElement2,selectElement,firstNode;
            domElment=this.constructor.createDiv(['hd-headertools']);
            divElement1=this.constructor.createDiv(['headertools-left-region']);
            divElement2=this.constructor.createDiv(['headertools-right-region ']);
            divElement3=this.constructor.createDiv(['hd-headertools-logo', 'headertools-position']);
            divElement4=this.constructor.createDiv(['hd-headertools-pc','headertools-position',' headertools-icon']);
            divElement5=this.constructor.createDiv(['hd-headertools-phone','headertools-position','headertools-icon']);
            divElement6=this.constructor.createDiv(['hd-headertools-page','headertools-position']);
            divElement7=this.constructor.createDiv(['headertools-undo','headertools-position','headertools-icon']);
            divElement8=this.constructor.createDiv(['headertools-redo','headertools-position','headertools-icon']);
            divElement9=this.constructor.createDiv(['headertools-preview','headertools-position'],null,'Preview');
            divElement10=this.constructor.createDiv(['headertools-save','headertools-position'],null,'Save');
            selectElement=this.constructor.createSelect(null,null,['Home']);
            spanElement1=this.constructor.createSpan(['headertools-icon']);
            spanElement2=this.constructor.createSpan(['headertools-icon']);
            divElement9.appendChild(spanElement1);
            divElement10.appendChild(spanElement2);
            divElement6.appendChild(selectElement);
            ...
            divElement2.appendChild(divElement7);
            divElement2.appendChild(divElement8);
            divElement2.appendChild(divElement9);
            divElement2.appendChild(divElement10);
            domElment.appendChild(divElement1);
            domElment.appendChild(divElement2);
            firstNode=document.body.firstChild;
            firstNode.nextSibling.style.top=60+'px';
            document.body.insertBefore(domElment,firstNode);
            var undo=new  ContentTools.ToolUI(ContentTools.ToolShelf.fetch('undo'));
            undo.mount(divElement7);
            undo.disabled(true);
            undo.addEventListener('applied', (function(_this) {
                return function() {
                    return _this.updateTools();
                };
            })(this));
            var redo=new  ContentTools.ToolUI(ContentTools.ToolShelf.fetch('redo'));
            redo.mount(divElement8);
            redo.disabled(true);
            redo.addEventListener('applied', (function(_this) {
                return function() {
                    return _this.updateTools();
                };
            })(this));
            divElement9.addEventListener('click',(function (_this) {
                return function () {
                    var preview=ContentTools.EditorApp.get();
                    preview.preview();
                }
            })(this));
            divElement10.addEventListener('click',(function (_this) {
                return function () {
                    var saved=ContentTools.EditorApp.get();
                    saved.saved();
                }
            })(this));
            return this._addDOMEventListeners();
        ...
        HeaderTools.prototype._addDOMEventListeners = function() {
    ...
    ContentTools.MoveInElement=(function (_super) {
        __extends(MoveInElement,_super);
        function MoveInElement(number,name,domElement) {
            MoveInElement.__super__.constructor.call(this);
            this._number=number;
            this._name=name;
            this._domSection=domElement;
            this._domElement=null;
        };
        MoveInElement.prototype.show=function () {
            this.mount();
        };
        MoveInElement.prototype.mount=function () {
            var divElement,spanElement;
            this._domElement=this.constructor.createDiv(['hd-move-in-section'],null,this._number+'.'+this._name);
            spanElement=this.constructor.createSpan(null,null,"|");
            divElement=this.constructor.createDiv(['hd-section-select']);
            this._domElement.appendChild(spanElement);
            this._domElement.appendChild(divElement);
            this._domSection.appendChild(this._domElement);
            console.log(this._domElement)
        };
        MoveInElement.prototype.unmount=function () {
            this._domSection.removeChild(this._domElement);
        }

        return MoveInElement;
    })(ContentTools.WidgetUI);
    ContentTools.Section=(function(_super){
        __extends(Section,_super);
        var SectionItem={
            'Banner':['Edit','Style','Setting'],
            'Services':['Edit','Style','Setting']
        };
        function Section(queryElement){
            /*this.constructor._contain=__bind(this.constructor._contain,this);*/
            this._onStopDragging = __bind(this._onStopDragging, this);
            this._onStartDragging = __bind(this._onStartDragging, this);
            this._onDrag = __bind(this._onDrag, this);
            Section.__super__.constructor.call(this);
            this._queryElement=queryElement;
            this._dragging = false;
            this._sectionName=null;//section的名称
            this._sectionNumber=null;
            this._domSection=null;
            this._regionItemList=[];//data-name,data-type的集合
            this._regionList=null;//data-region-num的顺序数组
            this._statement='empty';
            this._switchstate='on';
            this._sectionList={};
            this._editui=null;
            this.listenToDoc(this._queryElement);
            this._oldClickName=null;
            this._oldMoveName=null;
            this._navbar=null;
        };


        //根据传入的参数，获取文档中含有该参数属性的所有元素，并添加监听事件
        Section.prototype.listenToDoc=function(queryElement){
            var allDomSection,that=this;
            allDomSection=document.querySelectorAll("*["+queryElement+"]");//获取含有该参数的所有元素，计划定义为‘data-section’
            this._allDomSection=allDomSection;
            if(allDomSection){
                for(var i=0;i<allDomSection.length;i++){//遍历获取的所以元素
                    allDomSection[i].setAttribute('data-section-number',i+1);
                    this._sectionClick=(function(i){//逐个添加点击事件
                        return function(ev){
                            ev.preventDefault();
                            ev.stopPropagation();
                            that._sectionName=allDomSection[i].getAttribute(queryElement);//获取点击区域的data-section的值
                            /*if(that._oldClickName===that._sectionName){//比较本次点击的sectionName是否与上次点击的相同
                             console.log('hahaha')
                             return;//相同则直接返回
                             };*/
                            /*if(that._editui){//不相同则判断是否加载了子UI，加载则隐藏子UI
                             console.log('隐藏')
                             };*/
                            /*if(that._domElement){//如果已存在this._domElement,则移除此this._domElement
                             that.unmount();
                             };*/
                            var control=ContentTools.EditorApp.get();
                            control.controlstate('exitstate');
                            control.controller();
                            that._domSection=document.querySelector("[data-section="+that._sectionName+"]");//获取整个区域的DOM;
                            if(that._domSection){
                                that.sectionRegion();//DOM区域存在，则调.sectionRegion方法加载子列表
                                var control=ContentTools.EditorApp.get();
                                control.controlstate('edittool');
                                control.controller();
                            };
                            //触发edittool监听，调用show()方法
                            /* that._oldClickName=that._sectionName;//存储本次点击的sectionName*/
                        }
                    })(i)
                    allDomSection[i].addEventListener('click',this._sectionClick);
                    this._sectionMouseenter=(function (i) {
                        return function (ev) {

                            ev.preventDefault();
                            that._sectionName=allDomSection[i].getAttribute(queryElement);//获取点击区域的data-section的值
                            that._domSection=document.querySelector("[data-section="+that._sectionName+"]");
                            that._sectionNumber=i+1;
                            /*if(that._oldMoveName===that._sectionName){//比较本次点击的sectionName是否与上次点击的相同
                             return;//相同则直接返回
                             };*/
                            var control=ContentTools.EditorApp.get();
                            control.controlstate('section');
                            control.controller();
                            that._oldMoveName=that._sectionName;
                        }
                    })(i);
                    allDomSection[i].addEventListener('mouseenter',this._sectionMouseenter);
                    this._sectionMouseleave=(function (i) {
                        return function (ev) {
                            ev.preventDefault();
                            var control=ContentTools.EditorApp.get();
                            control.controlstate('mouse-out');
                            control.controller();
                        }
                    })(i);
                    allDomSection[i].addEventListener('mouseleave',this._sectionMouseleave);
                }
            }

        };

        Section.prototype.sectionitem=function () {
            return SectionItem;
        }
        //获取this._domSection下的子项
        Section.prototype.sectionRegion=function(){
            var regionList,regionItems,allRegionList=[],regionNumList=[];
            regionList=this._domSection.querySelectorAll('[data-region-num]');//获取点击的DOM区域下的所有包含data-region-num属性的子DOM集合；
            for(var i=0;i<regionList.length;i++){
                var regionNum,sectionList=[];
                regionNum=regionList[i].getAttribute('data-region-num');//获取子DOM的data-region-num属性的值
                regionNumList.push(regionNum);//将data-region-num的值集成到数组
                regionItems=regionList[i].querySelectorAll('[data-name]');//获取每个子DOM下含有data-name的元素
                for(var j=0;j<regionItems.length;j++){
                    var regionItemList={};
                    regionItemList.name=regionItems[j].getAttribute('data-name');
                    regionItemList.type=regionItems[j].getAttribute('data-type');
                    sectionList.push(regionItemList);
                }
                this._sectionList[regionNum]=sectionList;
            }
            this._regionList=regionNumList;

        };
        //返回点击的data-section的值；
        Section.prototype.sectionName=function(){
            return this._sectionName;
        };
        //返回所有data-name,data-type的值的集合对象
        Section.prototype.regionItemList=function(){
            return this._regionItemList;
        };
        //返回所有data-region-num的值的数组
        Section.prototype.regionList=function(){
            return this._regionList;
        };
        Section.prototype.switchstate=function (state) {
            if (state === void 0) {
                return this._switchstate;
            }
            if (this._switchstate === state) {
                return;
            }
        ...
        Section.prototype.isDragging = function() {
            return this._dragging;
        };
        Section.prototype.mount=function(){
            var restore,position,coord;
            if(this._sectionName){
                this._domElement=this.navList(eval("SectionItem."+this._sectionName));
                restore = window.sessionStorage.getItem('ct-position');
        ...
        Section.prototype.unmount = function() {
            Section.__super__.unmount.call(this);
            return this._domElement=null;
        };

        Section.prototype.navList=function(list){
            var domElement,that=this,navElement,maxNum,divElement;
            domElement=this.constructor.createDiv(['hd-editor']);
            this._navbar=this.constructor.createNavStyle(this._sectionName);
            /* this._navbar.getElementsByClassName('hd-editor-nav-exit')[0].addEventListener('click',(function () {
             return function (ev) {
             ev.stopPropagation();
             var control=ContentTools.EditorApp.get();
             control.controlstate('exitstate');
            ...
            domElement.appendChild(this._navbar);
            divElement=this.constructor.createDiv();
            maxNum=this._regionList;
            for(var i=0;i<list.length;i++){
                var menuElement;
                menuElement=this.constructor.createListStyle(list[i]);
                divElement.appendChild(menuElement);
                menuElement.addEventListener('click',(function(i){
                    return function (ev) {
                        ev.preventDefault();
                        console.log('1111111111111111111111')
                        var control=ContentTools.EditorApp.get();
                        control.controlstate(list[i]+'state');
                        control.controller();
                    }
                })(i))
        ...
        Section.prototype.state = function(state) {
            if (state === void 0) {
                return this._statement;
            }
            if (this._statement === state) {
                return;
            }
            if (!this.dispatchEvent(this.createEvent('statechange', {
                    state: state
        ...
        Section.prototype._removeSectionListen=function () {
            var allDomSection=this._allDomSection;
            if(allDomSection){
                for(var i=0;i<allDomSection.length;i++){//遍历获取的所以元素
                    allDomSection[i].setAttribute('data-section-number',i+1);
                    allDomSection[i].removeEventListener('click',this._sectionClick);
                    allDomSection[i].removeEventListener('mouseenter',this._sectionMouseenter);
                    allDomSection[i].removeEventListener('mouseleave',this._sectionMouseleave);
                }
            }
        }

        Section.prototype._addDOMEventListeners = function() {
            this._navbar.addEventListener('mousedown', this._onStartDragging);
            this._handleResize = (function(_this) {
                return function(ev) {
                    var containResize;
                    if (_this._resizeTimeout) {
        ...
        Section.prototype._contain = function() {
            var rect,rect2;
            if (!this.isMounted()) {
                return;
            }
            rect = this._domElement.getBoundingClientRect();
            rect2 = this._navbar.getBoundingClientRect();
            if (rect.left + rect.width > window.innerWidth) {
                this._domElement.style.left = "" + (window.innerWidth - rect.width) + "px";
            }
            if (rect.top + rect2.height > window.innerHeight) {
                this._domElement.style.top = "" + (window.innerHeight - rect2.height) + "px";
            }
            if (rect.left < 0) {
                this._domElement.style.left = '0px';
            }
            if (rect.top < 60) {
                this._domElement.style.top = '60px';
            }
            rect = this._domElement.getBoundingClientRect();
            return window.sessionStorage.setItem('ct-position', "" + parseInt(rect.left) + "," +parseInt(rect.top));
        };

        Section.prototype._removeDOMEventListeners = function() {
            if (this.isMounted()) {
                this._navbar.removeEventListener('mousedown', this._onStartDragging);
            }
        ...
        Section.prototype._onDrag = function(ev) {
            ContentSelect.Range.unselectAll();
        ...
        Section.prototype._onStartDragging = function(ev) {
            var rect;
            ev.preventDefault();
            if (this.isDragging()) {
                return;
        ...
        Section.prototype._onStopDragging = function(ev) {
            if (!this.isDragging()) {
                return;
            }
            this._contain();
    ...
    ContentTools.Solitary=(function(_super){
        __extends(Solitary,_super);
        function Solitary(queryElement) {
            this.listenToDoc(queryElement);
            this._controller=null;
        }
        Solitary.prototype.listenToDoc=function(queryElement){
            var allDomSection,that=this;
            allDomSection=document.querySelectorAll("*["+queryElement+"]");//获取含有该参数的所有元素，计划定义为‘data-section’
            for(var i=0;i<allDomSection.length;i++){//遍历获取的所以元素
                if(allDomSection[i].getAttribute(queryElement)==='Image'){
                    continue;
                }
                var clickevent;
                allDomSection[i].addEventListener('click',(function(i){//逐个添加点击事件
                    return function(ev){
                        ev.preventDefault();
                        ev.stopPropagation();
                        clearTimeout(clickevent);
                ...
                allDomSection[i].addEventListener('dblclick',(function(i){//逐个添加点击事件
                    return function(ev){
                        ev.preventDefault();
                        ev.stopPropagation();
                        clearTimeout(clickevent);
    ...
    ContentTools.EditUI=(function (_super) {
        __extends(EditUI, _super);

        function EditUI(domSection,maxNum,elementSet) {
            this._onStopDragging = __bind(this._onStopDragging, this);
            this._onStartDragging = __bind(this._onStartDragging, this);
            this._onDrag = __bind(this._onDrag, this);
            EditUI.__super__.constructor.call(this);
            this._switchNum=maxNum[0];
            this._domSection=domSection;
            this._regionNum=maxNum;
            this._back=false;
            this._maxNum=this._regionNum.length;
            this.sectionRegion();
            this._elementSet=elementSet;
            this._navbar=null;
            this._domElement=null;
        };
        EditUI.prototype.mount=function () {
            var restore,position,coord;
            var that=this;
            this._domElement = this.ui('Edit', this._switchNum);
            restore = window.sessionStorage.getItem('ct-position');
            if (restore/* && /^\d+,\d+$/.test(restore)*/) {
                position = (function() {
                    var _i, _len, _ref, _results;
            ...
            this.addEventListener('plus',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
                    document.body.removeChild(that._domElement);
                    that._domElement=that.ui('Edit',that._switchNum);
                    restore = window.sessionStorage.getItem('ct-position');
                    if (restore/* && /^\d+,\d+$/.test(restore)*/) {
                        position = (function() {
                            var _i, _len, _ref, _results;
                        ...
                        that._domElement.style.left = "" + position[0] + "px";
                        that._domElement.style.top = "" + position[1] + "px";
                        document.body.appendChild(that._domElement);
                        that._contain();
                    }
                    return that._addDOMEventListeners();
                }
            })(this));
            this.addEventListener('minus',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
                    document.body.removeChild(that._domElement);
                    that._domElement=that.ui('Edit',that._switchNum);
        ...
        EditUI.prototype.regionobj=function () {
            return this._sectionList[this._switchNum];
        };
        EditUI.prototype.show=function () {
            this.mount();
        };
        EditUI.prototype.ui=function (sectionName,num) {
            var domElement,menuElement,navElement,sectionList,region,regionElement;
            domElement=this.constructor.createDiv(['hd-editor']);
            this._navbar=this.constructor.createListNavStyle(sectionName);
            domElement.appendChild(this._navbar);
            sectionList=this._sectionList[num];
            region=this._domSection.querySelector('[data-region-num="'+num+'"]');
            regionElement=this.createRegionbox(num);
            domElement.appendChild(regionElement);
            if(sectionList){
                for(var i=0;i<sectionList.length;i++){
                    if(sectionList[i].type==='text'){
                        menuElement=this.constructor.createTextbox(sectionList[i].name,region,false,this._elementSet);
                        domElement.appendChild(menuElement);
                    }
                    if(sectionList[i].type==='img'){
                        menuElement=this.constructor.createImgbox(sectionList[i].name,region);
                        domElement.appendChild(menuElement);
                    }
                    if(sectionList[i].type==='button'){
                        menuElement=this.constructor.createBtnbox(sectionList[i].name,region,this._elementSet);
                        domElement.appendChild(menuElement);
                    }
                }
            };
        ...
        EditUI.prototype.plus=function () {
            if(this._switchNum>=this._maxNum){
                this._switchNum=1;
            }
            else {
                this._switchNum++;
            }
            if(!this.dispatchEvent(this.createEvent('plus'))){
                document.body.appendChild(this._domElement);
            }
            /*this.display();*/
        };
        EditUI.prototype.minus=function () {
            if(this._switchNum<=1){
                this._switchNum=this._maxNum;
            }
            else {
                this._switchNum--;
            }
            if(!this.dispatchEvent(this.createEvent('minus'))){
                document.body.appendChild(this._domElement);
            }
            /*this.display();*/
        };
        EditUI.prototype.add=function () {
            var domElement,parentElement,copyElement,restore;
            domElement=this._domSection.querySelector('[data-region-num="'+this._switchNum+'"]');
            copyElement=domElement.cloneNode(true);
            parentElement=domElement.parentNode;
            this._maxNum++;
            this._regionNum.push(this._maxNum);
            this._sectionList[this._maxNum]=this._sectionList[this._switchNum];
            copyElement.setAttribute('data-region-num',this._maxNum);
            parentElement.appendChild(copyElement);
            document.body.removeChild(this._domElement);
            var reflash=new ContentTools.EditorApp.get();
            reflash.start();
            this._elementSet=reflash._regions[reflash._section._sectionName]._elementSet;
            this._domElement=this.ui('Edit',this._maxNum);
            restore = window.sessionStorage.getItem('ct-position');
            if (restore/* && /^\d+,\d+$/.test(restore)*/) {
                position = (function() {
                    var _i, _len, _ref, _results;
        ...
        EditUI.prototype.delete=function () {
            var domElement,lastNum,oldNum,nextNum,parentElement,position,restore;
            domElement=this._domSection.querySelector('[data-region-num="'+this._switchNum+'"]');//获取要删除的元素，该元素为界面上的选择的区域
            parentElement=domElement.parentNode;
            position=this._switchNum-1;
            this._regionNum.splice(position,1);
            if(this._regionNum.length<1){
                return;
            }
            this._maxNum=this._regionNum.length;
            parentElement.removeChild(domElement);//在该元素的父节点中移除此元素
            lastNum=(this._regionNum.length+1).toString();
            for(var i=position;i<this._regionNum.length;i++){
                oldNum=(this._regionNum[i]-1).toString();
                nextNum=(this._regionNum[i]).toString();
                this._domSection.querySelector('[data-region-num="'+this._regionNum[i]+'"]').setAttribute('data-region-num',(this._regionNum[i]-1));
                this._sectionList[oldNum]=this._sectionList[nextNum];
                this._regionNum[i]=oldNum;
            }
            delete this._sectionList[lastNum];
            if(this._switchNum>1){
                this._switchNum--;
            }
            else {
        ...
        EditUI.prototype.moveback=function () {
            if(this._regionNum.length<2){
                return;
            }
            var initElement,aimElement,copyList,aimpos,initpos,copyAimElement,copyInitElement,parentElement,restore;
            if(this._switchNum>1){
                initpos=this._switchNum.toString();
                this._switchNum--;
                aimpos=this._switchNum.toString();
            }
            else {
                initpos=this._switchNum.toString();
                aimpos=this._regionNum.length.toString();
                this._switchNum=this._regionNum.length;
            }
            initElement=this._domSection.querySelector('[data-region-num="'+initpos+'"]');
            parentElement=initElement.parentNode;
            aimElement=this._domSection.querySelector('[data-region-num="'+aimpos+'"]');
            copyInitElement=initElement.cloneNode(true);
            copyAimElement=aimElement.cloneNode(true);
            copyInitElement.setAttribute('data-region-num',aimpos);
            copyAimElement.setAttribute('data-region-num',initpos);
            parentElement.replaceChild(copyInitElement,aimElement);
            parentElement.replaceChild(copyAimElement,initElement);
            copyList=this._sectionList[initpos];
            this._sectionList[initpos]=this._sectionList[aimpos];
            this._sectionList[aimpos]=copyList;
            document.body.removeChild(this._domElement);
            var reflash=new ContentTools.EditorApp.get();
            reflash.start();
            this._elementSet=reflash._regions[reflash._section._sectionName]._elementSet;
        ...
        EditUI.prototype.movenext=function () {
            if(this._regionNum.length<2){
                return;
            }
            var initElement,aimElement,copyList,aimpos,initpos,copyAimElement,copyInitElement,parentElement,restore;
            if(this._switchNum<this._regionNum.length){
                initpos=this._switchNum.toString();
                this._switchNum++;
                aimpos=this._switchNum.toString();
            }
                ...
                this._switchNum=1;
                aimpos=this._switchNum.toString();
                initpos=this._regionNum.length.toString();
        ...
        EditUI.prototype.createRegionbox=function (regionNum) {
            var domElement,spanElement1,spanElement2,divElement1,divElement2,dlElement1,dlElement2,dlElement3,dlElement4,that=this;
            domElement=this.constructor.createDiv(['hd-editor-handle_region']);
            divElement1=this.constructor.createDiv(['region_name'],null,'Region '+regionNum);
            divElement2=this.constructor.createDiv(['regions_ctrl']);
            spanElement1=this.constructor.createSpan(['region_left'],null,'&lt;');
            ...
            dlElement3.addEventListener('click',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
                    return _this.add();
                }
            })(this));
            dlElement4.addEventListener('click',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
                    return _this.delete();
                }
            })(this));
            dlElement1.addEventListener('click',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
                    return _this.moveback();
                }
            })(this));
            dlElement2.addEventListener('click',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
        ...
        EditUI.prototype.sectionRegion=function(){
            var regionList,regionItems,allRegionList=[],regionNumList=[];
            regionList=this._domSection.querySelectorAll('[data-region-num]');//获取点击的DOM区域下的所有包含data-region-num属性的子DOM集合；
            for(var i=0;i<regionList.length;i++){
                var regionNum,sectionList=[];
                regionNum=regionList[i].getAttribute('data-region-num');
                regionNumList.push(regionNum);

                regionItems=regionList[i].querySelectorAll('[data-name]');//获取子DOM下含有data-name的元素
                for(var j=0;j<regionItems.length;j++){
                    var regionItemList={};
                    regionItemList.name=regionItems[j].getAttribute('data-name');
                    regionItemList.type=regionItems[j].getAttribute('data-type');
        ...
        EditUI.prototype.display=function () {
            for(var i=0;i<this._regionNum.length;i++){
                if(this._regionNum[i]==this._switchNum){
                    this._domSection.querySelector('[data-region-num="'+this._regionNum[i]+'"]').style.display='block';
                }
                else {
                    this._domSection.querySelector('[data-region-num="'+this._regionNum[i]+'"]').style.display='none';
                }
            }
        };
        EditUI.prototype.isDragging = function() {
            return this._dragging;
        };

        EditUI.prototype._addDOMEventListeners = function() {
            this._navbar.addEventListener('mousedown', this._onStartDragging);
            this._handleResize = (function(_this) {
                return function(ev) {
                    var containResize;
        ...
        EditUI.prototype._contain = function() {
            var rect,rect2;
            if (!this.isMounted()) {
                return;
            }
            ...
            rect = this._domElement.getBoundingClientRect();
            return window.sessionStorage.setItem('ct-position', "" + rect.left + "," + rect.top);
        };

        EditUI.prototype._removeDOMEventListeners = function() {
            if (this.isMounted()) {
                this._navbar.removeEventListener('mousedown', this._onStartDragging);
            }
            window.removeEventListener('keydown', this._handleKeyDown);
        ...
        EditUI.prototype._onDrag = function(ev) {
            ContentSelect.Range.unselectAll();
            this._domElement.style.left = "" + (ev.clientX - this._draggingOffset.x) + "px";
            return this._domElement.style.top = "" + (ev.clientY - this._draggingOffset.y) + "px";
        };

        EditUI.prototype._onStartDragging = function(ev) {
            var rect;
            ev.preventDefault();
            if (this.isDragging()) {
                return;
        ...
        EditUI.prototype._onStopDragging = function(ev) {
            if (!this.isDragging()) {
                return;
            }
            this._contain();
        ...
        EditUI.prototype.domElement=function () {
            return this._domElement;
        }
        return EditUI;
    })(ContentTools.Section);
    ContentTools.StyleUI=(function (_super) {
        __extends(StyleUI, _super);

        function StyleUI(domSection,maxNum,elementSet) {
            this._onStopDragging = __bind(this._onStopDragging, this);
            this._onStartDragging = __bind(this._onStartDragging, this);
            this._onDrag = __bind(this._onDrag, this);
        ...
        StyleUI.prototype.mount=function () {
            var restore,position,coord;
            /*this.sectionRegion();*/
            var that=this;
            /*if(!this.dispatchEvent(this.createEvent('numchange'))) {
             this._switchNum = this._regionList[0];*/
            this._domElement = this.ui('Style', this._switchNum);
            /*}*/
            ...
            this.addEventListener('plus',(function (_this) {
                return function (ev) {
                    ev.preventDefault();
                    document.body.removeChild(that._domElement);
                    that._domElement=that.ui('Style',that._switchNum);
                    restore = window.sessionStorage.getItem('ct-position');
                    if (restore/* && /^\d+,\d+$/.test(restore)*/) {
                        position = (function() {
                            var _i, _len, _ref, _results;
        ...
        StyleUI.prototype.regionobj=function () {
            return this._sectionList[this._switchNum];
        };
        StyleUI.prototype.show=function () {
            this.mount();
        };
        StyleUI.prototype.ui=function (sectionName,num) {
            var domElement,menuElement,navElement,sectionList,region,regionElement,sliderElement;
            domElement=this.constructor.createDiv(['hd-editor']);
            this._navbar=this.constructor.createListNavStyle(sectionName);
            domElement.appendChild(this._navbar);
            sectionList=this._sectionList[num];
            ...
            if(sectionList){
                for(var i=0;i<sectionList.length;i++){
                    if(sectionList[i].type==='text'){
                        menuElement=this.constructor.createColorbox(sectionList[i].name,region,this._elementSet);
                        domElement.appendChild(menuElement);
                        sliderElement=this.constructor.Slider(sectionList[i].name,region,this._elementSet);
                        domElement.appendChild(sliderElement);
                    }
                    /* if(sectionList[i].type==='img'){
                     menuElement=this.constructor.createImgbox(sectionList[i].name,region);
                     domElement.appendChild(menuElement);
        ...
        StyleUI.prototype.plus=function () {
            if(this._switchNum>=this._maxNum){
                this._switchNum=1;
            }
            else {
        ...
        StyleUI.prototype.minus=function () {
            if(this._switchNum<=1){
                this._switchNum=this._maxNum;
            }
            else {
        ...
        StyleUI.prototype.add=function () {
            var domElement,parentElement,copyElement;
            domElement=this._domSection.querySelector('[data-region-num="'+this._switchNum+'"]');
            copyElement=domElement.cloneNode(true);
            parentElement=domElement.parentNode;
            this._maxNum++;
            ...
            parentElement.appendChild(copyElement);
            document.body.removeChild(this._domElement);
            this._domElement=this.ui('Style',this._maxNum);
            document.body.appendChild(this._domElement);
            this._switchNum=this._maxNum;
            this.display();
        };
        StyleUI.prototype.delete=function () {
            var domElement,lastNum,oldNum,nextNum,parentElement,position;
            domElement=this._domSection.querySelector('[data-region-num="'+this._switchNum+'"]');//获取要删除的元素，该元素为界面上的选择的区域
            parentElement=domElement.parentNode;
            position=this._switchNum-1;
            this._regionNum.splice(position,1);
        ...
        StyleUI.prototype.moveback=function () {
            if(this._regionNum.length<2){
                return;
            }
            var initElement,aimElement,copyList,aimpos,initpos,copyAimElement,copyInitElement,parentElement;
            if(this._switchNum>1){
                initpos=this._switchNum.toString();
                this._switchNum--;
                aimpos=this._switchNum.toString();
        ...
        StyleUI.prototype.movenext=function () {
            if(this._regionNum.length<2){
                return;
            }
            var initElement,aimElement,copyList,aimpos,initpos,copyAimElement,copyInitElement,parentElement;
        ...
        StyleUI.prototype.createRegionbox=function (regionNum) {
            var domElement,spanElement1,spanElement2,divElement1,divElement2,dlElement1,dlElement2,dlElement3,dlElement4,that=this;
            domElement=this.constructor.createDiv(['hd-editor-handle_region']);
            divElement1=this.constructor.createDiv(['region_name'],null,'Region '+regionNum);
            divElement2=this.constructor.createDiv(['regions_ctrl']);
        ...
        StyleUI.prototype.sectionRegion=function(){
            var regionList,regionItems,allRegionList=[],regionNumList=[];
            regionList=this._domSection.querySelectorAll('[data-region-num]');//获取点击的DOM区域下的所有包含data-region-num属性的子DOM集合；
            for(var i=0;i<regionList.length;i++){
                var regionNum,sectionList=[];
        ...
        StyleUI.prototype.display=function () {
            for(var i=0;i<this._regionNum.length;i++){
                if(this._regionNum[i]==this._switchNum){
                    this._domSection.querySelector('[data-region-num="'+this._regionNum[i]+'"]').style.display='block';
                }
        ...
        StyleUI.prototype.domElement=function () {
            return this._domElement;
        }
        StyleUI.prototype.isDragging = function() {
            return this._dragging;
        };
        ...
        StyleUI.prototype._contain = function() {
            var rect,rect2;
            if (!this.isMounted()) {
                return;
            }
        ...
        StyleUI.prototype._removeDOMEventListeners = function() {
            if (this.isMounted()) {
                this._navbar.removeEventListener('mousedown', this._onStartDragging);
            }
            window.removeEventListener('keydown', this._handleKeyDown);
        ...
        StyleUI.prototype._onDrag = function(ev) {
            ContentSelect.Range.unselectAll();
            this._domElement.style.left = "" + (ev.clientX - this._draggingOffset.x) + "px";
            return this._domElement.style.top = "" + (ev.clientY - this._draggingOffset.y) + "px";
        };

        StyleUI.prototype._onStartDragging = function(ev) {
            var rect;
            ev.preventDefault();
            if (this.isDragging()) {
                return;
        ...
        StyleUI.prototype._onStopDragging = function(ev) {
            if (!this.isDragging()) {
                return;
            }
            this._contain();
    ...
    ContentTools.SettingUI=(function (_super) {
        __extends(SettingUI, _super);

        function SettingUI(domSection,maxNum) {
            this._onStopDragging = __bind(this._onStopDragging, this);
            this._onStartDragging = __bind(this._onStartDragging, this);
            this._onDrag = __bind(this._onDrag, this);
            StyleUI.__super__.constructor.call(this);
        ...
        SettingUI.prototype.mount=function () {
            var restore,position,coord;
            var that=this;
            this._domElement = this.ui('Setting', this._switchNum);
        ...
        SettingUI.prototype.regionobj=function () {
            return this._sectionList[this._switchNum];
        };
        SettingUI.prototype.show=function () {
            this.mount();
        };
        SettingUI.prototype.ui=function (sectionName,num) {
            var domElement,menuElement,navElement,setElement;
            domElement=this.constructor.createDiv(['hd-editor']);
            this._navbar=this.constructor.createListNavStyle(sectionName);
            domElement.appendChild(this._navbar);
            setElement=this.constructor.Setting(this._domSection);
            domElement.appendChild(setElement);
            return domElement;
        };
        SettingUI.prototype.display=function () {
            for(var i=0;i<this._regionNum.length;i++){
                if(this._regionNum[i]==this._switchNum){
                    this._domSection.querySelector('[data-region-num="'+this._regionNum[i]+'"]').style.display='block';
                }
        ...
        SettingUI.prototype.domElement=function () {
            return this._domElement;
        }
        SettingUI.prototype.isDragging = function() {
            return this._dragging;
        };
        SettingUI.prototype._addDOMEventListeners = function() {
            this._navbar.addEventListener('mousedown', this._onStartDragging);
            this._handleResize = (function(_this) {
                return function(ev) {
                    var containResize;
        ...
        SettingUI.prototype._contain = function() {
            var rect,rect2;
            if (!this.isMounted()) {
                return;
            }
        ...
        SettingUI.prototype._removeDOMEventListeners = function() {
            if (this.isMounted()) {
                this._navbar.removeEventListener('mousedown', this._onStartDragging);
            }
            window.removeEventListener('keydown', this._handleKeyDown);
        ...
        SettingUI.prototype._onDrag = function(ev) {
            ContentSelect.Range.unselectAll();
            this._domElement.style.left = "" + (ev.clientX - this._draggingOffset.x) + "px";
            return this._domElement.style.top = "" + (ev.clientY - this._draggingOffset.y) + "px";
        };

        SettingUI.prototype._onStartDragging = function(ev) {
            var rect;
            ev.preventDefault();
            if (this.isDragging()) {
                return;
        ...
        SettingUI.prototype._onStopDragging = function(ev) {
            if (!this.isDragging()) {
                return;
            }
            this._contain();
    ...
    ContentTools.SingleElement=(function (_super) {
        __extends(SingleElement,_super);
        function SingleElement(name,aimElement) {
            this._onStopDragging = __bind(this._onStopDragging, this);
            this._onStartDragging = __bind(this._onStartDragging, this);
            this._onDrag = __bind(this._onDrag, this);
            this.listenToDoc();
            ...
            this._regionNum=null;
        }
        SingleElement.prototype.listenToDoc=function () {
            var allElement,that=this;
            allElement=document.querySelectorAll("*[data-name]");
            for(var i=0;i<allElement.length;i++){
                if(allElement[i].getAttribute('data-type')!=='img'){
                    allElement[i].addEventListener('click',(function (i) {
                        return function (ev) {
                            ev.preventDefault();
                            ev.stopPropagation();
                            var parentArr=[];
                            ...
                            for(var j=0;j<parentArr.length;j++){
                                var sectionName,regionNum;
                                if(parentArr[j].getAttribute('data-region-num')){
                                    regionNum=parentArr[j].getAttribute('data-region-num');
                                }
                                if(parentArr[j].getAttribute('data-section')){
                                    sectionName=parentArr[j].getAttribute('data-section');
                                }
                            }

                            that._element=this;
                            that._regionNum=regionNum;
                            that._elementType=this.getAttribute('data-type');
                            var control=ContentTools.EditorApp.get();
                            control.controlstate('element');
                            control.controller();
                        }
        ...
        SingleElement.prototype.mount=function (aimElement,elementType,elementSet) {
            var restore;
            this._elementSet=elementSet;
            this._domElement=this.elementUI(aimElement,elementType);
            restore = window.sessionStorage.getItem('ct-position');
            if (restore/* && /^\d+,\d+$/.test(restore)*/) {
                position = (function() {
                    var _i, _len, _ref, _results;
        ...
        SingleElement.prototype.elementUI=function (aimElement,elementType) {
            var domElement,navElement,scrollElement,textElement,borderElement,effectElement,backgroundElement;
            domElement=this.constructor.createDiv(['hd-editor']);
            this._navbar=this.constructor.createNavStyle(elementType);
            domElement.appendChild(this._navbar);
            scrollElement=this.constructor.createDiv(['hd-scroll']);
            switch (elementType){
                case 'text':
                    textElement=this.textStyle(aimElement);
                    borderElement=this.borderStyle(aimElement);
                    effectElement=this.effects(aimElement);
                    backgroundElement=this.background(aimElement);
                    break;
                case  'button':
                    textElement=this.textStyle(aimElement);
                    borderElement=this.borderStyle(aimElement);
            ...
            if(textElement){
                scrollElement.appendChild(textElement);
            }
            if(borderElement){
                scrollElement.appendChild(borderElement);
            }
            if(effectElement){
                scrollElement.appendChild(effectElement);
            }
            if(backgroundElement){
                scrollElement.appendChild(backgroundElement);
            }
            domElement.appendChild(scrollElement);
            return domElement;
        };

        SingleElement.prototype.textStyle=function (aimElement) {
            var domElement,textColor,textSize;
            domElement=this.constructor.createDiv();
            textSize=this.constructor.Slider2(aimElement, 'fontSize','TextSize',0,50,0,this._elementSet,this._regionNum);
            domElement.appendChild(textSize);
            textColor=this.constructor.createColorbox2(aimElement,'color','TextColor',this._elementSet,this._regionNum);
            domElement.appendChild(textColor);
            return domElement;
        };

        SingleElement.prototype.borderStyle=function (aimElement) {
            var domElement,borderColor,borderWidth,borderRadius;
            domElement=this.constructor.createDiv();
            borderWidth=this.constructor.Slider2(aimElement, 'border','BorderWidth',0,20,0,this._elementSet,this._regionNum);
            domElement.appendChild(borderWidth);
            borderRadius=this.constructor.Slider2(aimElement, 'borderRadius','BorderRadius',0,20,0,this._elementSet,this._regionNum);
            domElement.appendChild(borderRadius);
            borderColor=this.constructor.createColorbox2(aimElement,'borderColor','BorderColor',this._elementSet,this._regionNum);
            domElement.appendChild(borderColor);
            return domElement;
        };

        SingleElement.prototype.effects=function (aimElement) {
            var domElement,opacity,shadow,glow,innerShadow,innerGlow;
            domElement=this.constructor.createDiv();
            opacity=this.constructor.Slider2(aimElement, 'opacity','Opacity',0,20,0);
            domElement.appendChild(opacity);
            shadow=this.constructor.Slider2(aimElement, 'boxShadow','Shadow',0,20,0);
            domElement.appendChild(shadow);
            /*glow=this.constructor.Slider2(aimElement, aimAttr,sliderName);
             domElement.appendChild(glow);
             innerShadow=this.constructor.Slider2(aimElement, aimAttr,sliderName);
             domElement.appendChild(innerShadow);
             innerGlow=this.constructor.Slider2(aimElement, aimAttr,sliderName);
             domElement.appendChild(innerGlow);*/
            return domElement;
        };

        SingleElement.prototype.background=function (aimElement, aimAttr,sliderName) {
            var domElement,backgroundColor;
            domElement=this.constructor.createDiv();
            backgroundColor=this.constructor.createColorbox2(aimElement,'backgroundColor','Background');
            domElement.appendChild(backgroundColor);
            return domElement;
        };

        SingleElement.prototype.isDragging = function() {
            return this._dragging;
        };

        SingleElement.prototype._addDOMEventListeners = function() {
            this._navbar.addEventListener('mousedown', this._onStartDragging);
            this._handleResize = (function(_this) {
                return function(ev) {
                    var containResize;
        ...
        SingleElement.prototype._contain = function() {
            var rect,rect2;
            if (!this.isMounted()) {
                return;
            }
        ...
        SingleElement.prototype._removeDOMEventListeners = function() {
            if (this.isMounted()) {
                this._navbar.removeEventListener('mousedown', this._onStartDragging);
            }
            window.removeEventListener('keydown', this._handleKeyDown);
        ...
        SingleElement.prototype._onDrag = function(ev) {
            ContentSelect.Range.unselectAll();
            this._domElement.style.left = "" + (ev.clientX - this._draggingOffset.x) + "px";
            return this._domElement.style.top = "" + (ev.clientY - this._draggingOffset.y) + "px";
        };

        SingleElement.prototype._onStartDragging = function(ev) {
            var rect;
            ev.preventDefault();
            if (this.isDragging()) {
                return;
        ...
        SingleElement.prototype._onStopDragging = function(ev) {
            if (!this.isDragging()) {
                return;
            }
            this._contain();
  ...
  //ContentTools.ColorPickerUI
  ContentTools.ColorPickerUI=(function (_super) {
      __extends(ColorPickerUI, _super);
      function ColorPickerUI() {
          ColorPickerUI.__super__.constructor.call(this);
          this._onStopDragging = __bind(this._onStopDragging, this);
          this._onStartDragging = __bind(this._onStartDragging, this);
          this._onDrag = __bind(this._onDrag, this);
          this._dragging = false;
          this._draggingOffset = null;
          /*this._domColorDrag=null;
          this._domColor=null;*/
      };

      ColorPickerUI.prototype.mount=function () {
          var coord, position, restore;
          this._domElement=this.constructor.createDiv(['cp-all']);
          this.parent().domElement().appendChild(this._domElement);
          this._domColorDrag= this.constructor.createDiv(['cp-colordrag']);
          this._domElement.appendChild(this._domColorDrag);
          this._domColor = this.constructor.createDiv(['cp-default']);
          this._domElement.appendChild(this._domColor);
          restore = window.localStorage.getItem('ct-colorpicker-position');
          if (restore && /^\d+,\d+$/.test(restore)) {
              position = (function () {
                  var _i, _len, _ref, _results;
                  _ref = restore.split(',');
      ...
      ColorPickerUI.prototype.isDragging = function() {
          return this._dragging;
      };
      ColorPickerUI.prototype._removeDOMEventListeners = function() {
          if (this.isMounted()) {
              return this._domElement.removeEventListener('mousedown', this._onStartDragging);
          }
      };
      ColorPickerUI.prototype._addDOMEventListeners = function() {
          this._domColorDrag.addEventListener('mousedown', this._onStartDragging);
      }
      ColorPickerUI.prototype._onStartDragging = function(ev) {
          var rect;
          ev.preventDefault();
          if (this.isDragging()) {
              return;
      ...
      ColorPickerUI.prototype._onStopDragging = function(ev) {
          if (!this.isDragging()) {
              return;
          }
          this._contain();
      ...
      ColorPickerUI.prototype._onDrag = function(ev) {
          ContentSelect.Range.unselectAll();
          this._domElement.style.left = "" + (ev.clientX - this._draggingOffset.x) + "px";
          return this._domElement.style.top = "" + (ev.clientY - this._draggingOffset.y) + "px";
      };
      ColorPickerUI.prototype._contain = function() {
          var rect;
          if (!this.isMounted()) {
              return;
          }
          ...
          rect = this._domElement.getBoundingClientRect();
          return window.localStorage.setItem('ct-colorpicker-position', "" + rect.left + "," + rect.top);
      };
      ColorPickerUI.prototype.show=function () {
          ContentTools.on=true;
          var fadeIn;
          if(document.getElementsByClassName('cp-default').length<1){
              this.mount();
          }
          fadeIn = (function(_this) {
              return function() {
      ...
      ColorPickerUI.prototype.hide = function() {
          ContentTools.on=false;
          this._removeDOMEventListeners();
          this.parent().domElement().removeChild(document.getElementsByClassName('cp-all')[0]);
          return this._domElement=null;
      };
  ...
  _EditorApp = (function(_super) {
    ...
    function _EditorApp() {
      ...
      this._sectionState=null;
        this._controller=null;
        this._editui=null;
        this._styleui=null;
        this._substate=null;
        this._settingui=null;
        this._moveInElement=null;
        this._singleElement=null;
        this._singleElementType=null;
        this._oldSectionName=null;
    }
    ...
    _EditorApp.prototype.init = function(queryOrDOMElements, namingProp, fixtureTest, withIgnition) {
      ...
      this._headertools=new ContentTools.HeaderTools();
        this._headertools.mount();
      this._toolbox = new ContentTools.ToolboxUI(ContentTools.DEFAULT_TOOLS);
      this.attach(this._toolbox);
       /* this._controller=new ContentTools.Controller();*/
        this._inspector = new ContentTools.InspectorUI();
       /* this.attach(this._controller);*/
        this._section=new  ContentTools.Section('data-section');
        this.attach(this._section);


        this._solitary=new  ContentTools.Solitary('data-name');
        this.attach(this._solitary);
        /*this._texttool=new  ContentTools.TextTool('data-name');
        this.attach(this._texttool);*/
        ...
        this.history.watch();
        this._perElement=new ContentTools.SingleElement();
        this.attach(this._perElement);
        this._controlLis=(function (_this) {
            return function (ev) {
                ev.preventDefault();
                var statearr;
                if(_this._controller==='element') {
                    if (_this._editui) {
                        _this._editui.unmount();
                    }
                    if (_this._styleui) {
                        _this._styleui.unmount();
                    }
                    if (_this._settingui) {
                        _this._settingui.unmount();
                    }
                    if (_this._section) {
                        _this._section.unmount();
                    }
                    if (_this._perElement._domElement) {
                        _this._perElement.unmount();
                        _this._perElement.mount(_this._singleElement, _this._singleElementType,_this._regions[_this._section._sectionName]._elementSet);
                    }else {
                        if(_this._regions[_this._section._sectionName]._elementSet&&_this._singleElement){
                            _this._perElement.mount(_this._singleElement, _this._singleElementType,_this._regions[_this._section._sectionName]._elementSet);
                        }
                ...
                if(_this._controller==='edittool'){
                    /*if (_this._perElement._domElement) {
                        _this._perElement.unmount();
                    }*/
                    console.log(_this._section._domElement)
                    if(_this._section._domElement){
                        return;
                    }
                    console.log('here')
                    _this._section.show();
                    statearr=_this._section.sectionitem();
                    _this._substate=statearr[_this._section._sectionName];
                }
                if(_this._controller==='section'){
                    if(!_this._moveInElement){
                        _this._moveInElement=new ContentTools.MoveInElement(_this._section._sectionNumber,_this._section._sectionName,_this._section._domSection);
                        _this._moveInElement.show();
                    }
                }
                if(_this._controller==='mouse-out'){
                    if(_this._moveInElement){
                        _this._moveInElement.unmount();
                        _this._moveInElement=null;
                    }
                }
                if(_this._controller==='texttool'){
                    _this._toolbox.show();
                }
                if(_this._substate){
                    for(var i=0;i<_this._substate.length;i++){
                        if(_this._controller===(_this._substate[0]+'state')){
                            _this._editui=new ContentTools.EditUI(_this._section._domSection,_this._section._regionList,_this._regions[_this._section._sectionName]._elementSet);
                            _this._editui.show();
                            _this._section.unmount();
                            break;
                        }
                        else if(_this._controller===(_this._substate[1]+'state')){
                            _this._styleui=new ContentTools.StyleUI(_this._section._domSection,_this._section._regionList,_this._regions[_this._section._sectionName]._elementSet);
                            _this._styleui.show();
                            _this._section.unmount();
                            break;
                        }
                        else if(_this._controller===(_this._substate[2]+'state')){
                            _this._settingui=new ContentTools.SettingUI(_this._section._domSection,_this._section._regionList,_this._regions[_this._section._sectionName]._elementSet);
                            _this._settingui.show();
                            _this._section.unmount();
                            break;
                        }
                    }
                }
                if(_this._controller==='returnstate'){
                    if(_this._editui){
                        _this._editui.unmount();
                    }
                    if(_this._styleui){
                ...
                if(_this._controller==='exitstate'){
                    if(_this._section){
                        _this._section.unmount();
                        _this._section._oldClickName=null;
                    }
                    if(_this._editui){
                        _this._editui.unmount();
                    }
                ...
                if(_this._controller==='revert'){

                    if(_this._editui){
                        if(_this._editui.domElement()){
                            _this._editui.unmount();
                            _this._editui=new ContentTools.EditUI(_this._section._domSection,_this._section._regionList,_this._regions[_this._section._sectionName]._elementSet);
                            _this._editui.show();
                        }
                    ...
                    if(_this._styleui){
                        if(_this._styleui.domElement()){
                            _this._styleui.unmount();
                            _this._styleui=new ContentTools.StyleUI(_this._section._domSection,_this._section._regionList);
                            _this._styleui.show();
                        }
                    }

                    if(_this._settingui){
                        if(_this._settingui.domElement()){
                            _this._settingui.unmount();
                            _this._settingui.show();
                        }
                    }
                    if (_this._perElement._domElement) {
                        _this._perElement.unmount();
                        _this._perElement.mount(_this._singleElement, _this._singleElementType);
                    }
                }
            }
        })(this)
            this.addEventListener('control',this._controlLis);
        var  off=true;
        this.addEventListener('preview',(function (_this) {
            return function(ev) {
                ev.preventDefault();
                if(off){
                    _this.stop(true);

                    _this._section._removeSectionListen();
                    /*_this._section=null;*/
                    _this.removeEventListener('control',_this._controlLis);
                    if(_this._section){
                        _this._section.unmount();
                    }
                    ...
                    _this.addEventListener('control',_this._controlLis);
                    _this._section.listenToDoc('data-section');
                    _this._perElement.listenToDoc();
                    _this.start();
                    off=true;
                }
            }
        })(this));
        this.addEventListener('headersave',(function (_this) {
            return function(ev) {
                _this.save();
            }
        })(this));
      this._colorpicker=new ContentTools.ColorPickerUI();
      this.attach(this._colorpicker);
        ContentTools.hex=this._colorpicker;
      this._state = 'ready';
    ...
    _EditorApp.prototype.mount = function() {
        this.constructor.SectionName();
      /*this._domElement = this.constructor.createDiv(['cp-app']);
    ...
    _EditorApp.prototype.revertToSnapshot = function(snapshot, restoreEditable) {
      var child, name, region, _i, _len, _ref, _ref1,oneRegion,elementRegion,num,addressRegion,address,name1;
      ...
      for (name in _ref) {
        ...
        for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
          ...
          _ref[name]._addressSet.innerHTML=snapshot.regions[name];
        /*region.subElement().innerHTML = snapshot.regions[name];*/
      }
      if (restoreEditable) {
    ...
    _EditorApp.prototype._initRegions = function() {
      ...
      found = {};
      this._orderedRegions = [];
      _ref = this._domRegions;//所有含有data-editable属性的元素集合
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        domRegion = _ref[i];
        ...
        this._orderedRegions.push(name);
         /* console.log(this._regions[name].domElement())*/
      ...
      _EditorApp.prototype.controller=function () {
          if(!this.dispatchEvent(this.createEvent('control'))){
          }
      };
      _EditorApp.prototype.preview=function () {
          if(!this.dispatchEvent(this.createEvent('preview'))){
          }
      };
      _EditorApp.prototype.saved=function () {
          if(!this.dispatchEvent(this.createEvent('headersave'))){
          }
      };
      _EditorApp.prototype.controlstate=function(state) {
      ...
      _EditorApp.prototype.setSingleElement=function (element) {
         return this._singleElement=element;
      }
      _EditorApp.prototype.setSingleElementType=function (elementType) {
          return this._singleElementType=elementType;
      }
    return _EditorApp;

  })(ContentTools.ComponentUI);
   // ContentTools.EditorApp 
  ...
  ContentTools.History = (function() {
    ...
    History.prototype._store = function()  {
      var element, name, other_region, region, snapshot, _ref, _ref1,num,region1,reg,oneRegion;
      snapshot = {
        regions: {},
        selected: null
      };
      _ref = this._regions;
        for (name in _ref) {
            region = _ref[name];
            snapshot.regions[name] =region._domElement.innerHTML;
        }
     /* for (name in _ref) {
        region = _ref[name];
  ...
  //ContentTools.Tool
   ...
   /* ContentTools.Controller=(function (_super) {
        __extends(Controller,_super);
        function Controller() {
            Controller.__super__.constructor.call(this);
            this._state=null;
            this.addEventListener('elementtool',(function () {
                return function (ev) {
                    ev.preventDefault();
                }
            })())
        ...
        Controller.prototype.edittool=function () {
            if(!this.dispatchEvent(this.createEvent('edittool'))){
               return  this.state('edittool');
            }
        };
        Controller.prototype.sectiontool=function () {
            if(!this.dispatchEvent(this.createEvent('sectiontool'))){
                this.state()
            }
        };
        Controller.prototype.elementtool=function () {
            if(!this.dispatchEvent(this.createEvent('elementtool'))){
                this.state('elementtool');
                console.log(this._state)
            }
        };
        Controller.prototype.texttool=function () {
            if(!this.dispatchEvent(this.createEvent('texttool'))){
                this.state()
            }
        };
        return Controller;
   ...
   /* ContentTools.TextTool=(function(_super){
        __extends(TextTool,_super);
        function TextTool(queryElement) {
            this.listenToDoc(queryElement);
            this._controller=null;
        }
        TextTool.prototype.listenToDoc=function(queryElement){
            var allDomSection,that=this;
            allDomSection=document.querySelectorAll("*["+queryElement+"]");//获取含有该参数的所有元素，计划定义为‘data-section’
            for(var i=0;i<allDomSection.length;i++){//遍历获取的所以元素
                if(allDomSection[i].getAttribute(queryElement)==='Image'){
  ...
  ContentTools.Message=(function () {
      function Message(author,company,time) {
          this._author=author;
      ...
      Message.prototype.getCompany=function () {
          return this._company;
      }
      Message.prototype.getTime=function () {
          return this._time;
      }
      Message.prototype.getAll=function () {
          alert('Author:'+this._author+',Company:'+this._company+',Time:'+this._time);
      }
      return Message;
  })();

  ContentTools.DEFAULT_TOOLS[3].push('time');
